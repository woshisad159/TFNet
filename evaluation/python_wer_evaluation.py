...
def load_prediction1(fpath):
    file_info = open(fpath, 'r', encoding='utf-8').readlines()
    pre_dict = dict()
    for line in file_info:
        file_name, num, _, _, wd = line[:-1].split(" ")

        file_nameTmp = file_name + " " + str(num)

        if file_nameTmp not in pre_dict.keys():
            pre_dict[file_nameTmp] = [wd]
        else:
            pre_dict[file_nameTmp].append(wd)
...
def calculate_stats(gt, lstm_pred, conv_pred=None):
    ...
    for i in range(len(lstm_pred)):
        if "*" == lstm_pred[i][0]:
            cnt = cnt + 1
        # print(len(lstm_pred), (cnt + 1), lstm_pred[-1])
        if len(lstm_pred) == (cnt + 1) and "[EMPTY]" == lstm_pred[-1]:
            flag = False
            break
...
def wer_calculation(gt_path, primary_pred, isPrint, auxiliary_pred=None):
...
def wer_calculation1(gt_path, primary_pred, isPrint, auxiliary_pred=None):
    gt = load_groundtruth(gt_path)
    pred1 = load_prediction1(primary_pred)
    results_list = []

    lastNum = 1
    for fileid, sent in pred1.items():
        fileidList = fileid.split(" ")
        fileid = fileidList[0]
        num = int(fileidList[1])
        sent_stat = sent_evaluation(
            info=fileid, gt=gt[fileid],
            merge_same=True,
            lstm_prediction=sent,
            penalty={'ins': 3, 'del': 3, 'sub': 4},
        )
        if sent_stat['wer_lstm'] != 0 or sent_stat['cnt'] != 0:
            if num == 1:
                results_list.append(sent_stat['wer_lstm'] / sent_stat['cnt'] * 100)
            else:
                if num - lastNum == 1:
                    results_list.append(sent_stat['wer_lstm'] / sent_stat['cnt'] * 100)
                else:
                    for _ in range(num - lastNum - 1):
                        results_list.append(results_list[-1])
...
def wer_calculation2(gt_path, primary_pred, auxiliary_pred=None):
    gt = load_groundtruth(gt_path)
    pred1 = load_prediction(primary_pred)
    results_list = []
    if auxiliary_pred is not None:
